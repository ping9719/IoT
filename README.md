...
跨平台工业通讯库。包含主流的通讯方式（TCP、UDP、MQTT、USB、蓝牙……）和主流的通讯协议（Modbus、S7、CIP、MC、FINS……），开箱即用，简单方便。
...
[提交问题、提交 BUG](https://github.com/ping9719/IoT/issues)    
...
| Ping9719.IoT.Hid      | net45 ; netstandard2.0 ; net8.0 | 跨平台工业通讯库 Hid 扩展库。包含不常用的通信方式（USB、蓝牙、串口） |
...
### 二 <b>所有客户端协议</b>可快速切换为不同的方式，比如从 `TCP` 切换为 `USB` 
> 这里以 `ModbusRtu` 举例，默认只支持串口。但是如果你想实现 `ModbusRtuOverTcpClient`（使用 TCP 的方式走 `ModbusRtu` 协议），其他同理。 
...
//切换为USB方式。会沿用旧的属性，如果旧的已打开，会自动关闭旧的并打开新的
client.SetClient(new UsbHidClient(UsbHidClient.GetNames[0]));
```

### 三 客户端 `ClientBase` 包含丰富的功能，且代码一致性高。   
> 以下代码全部通用，包含 `TcpClient`、`SerialPortClient`、`UsbHidClient` 等……
```CSharp
...
client.ConnectionMode = ConnectionMode.AutoOpen;//自动打开。没有执行Open()时，每次发送和接收都会自动打开和关闭，比较适合需要短连接的场景；如需要临时的长连接，也可以调用Open()后再Close()。
client.ConnectionMode = ConnectionMode.AutoReconnection;//自动断线重连。在执行了Open()后，如果检测到断开后会自动打开，比较适合需要长连接的场景。调用Close()将不再重连。
...
### 四 返回类型统一为 `IoTResult`，不需要再单独使用 `Try` 来处理异常信息。
> `IoTResult<T>` 包含 `Value`，`IoTResult` 不包含 
```CSharp
var info = client.Read<bool>("abc");
if (info.IsSucceed)//应判断后再取值
   var val = info.Value;
else
